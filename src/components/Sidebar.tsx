        ...
        <ul className='flex flex-col py-4 text-grey-800'>
            ...
            return (
                <li key={id} className={`${navLink === title.toLocaleLowerCase() ? 'bg-sky-500 bg-opacity-60': 'hover:bg-sky-500 hover:bg-opacity-60'}`}>
                    <Link key={id} to={link}  onClick={()=>setSidebarOpen(!sidebarOpen)} className={`w-full text-xl p-4 flex gap-3 justify-start items-center transform duration-300 cursor-pointer ${navLink === title.toLocaleLowerCase() ? 'translate-x-2' : 'hover:translate-x-2'}`}>
